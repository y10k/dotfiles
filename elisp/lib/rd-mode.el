...
(defun rd-setup-keys ()
  ...
  (define-key rd-mode-map "\C-cv" 'rd-cite-region)
  (define-key rd-mode-map "\C-cie" 'rd-insert-emphasis)
  (define-key rd-mode-map "\C-cic" 'rd-insert-code)
  (define-key rd-mode-map "\C-civ" 'rd-insert-var)
  (define-key rd-mode-map "\C-cik" 'rd-insert-keyboard)
  (define-key rd-mode-map "\C-cii" 'rd-insert-index)
  (define-key rd-mode-map "\C-cir" 'rd-insert-ref)
  (define-key rd-mode-map "\C-ciu" 'rd-insert-reftourl)
  (define-key rd-mode-map "\C-cif" 'rd-insert-footnote)
  (define-key rd-mode-map "\C-cib" 'rd-insert-verb)
  (define-key rd-mode-map "\C-cy" 'rd-yank-as-url)
  (define-key rd-mode-map "\C-c\C-y" 'rd-yank-pop-as-url)
...
(defun rd-insert-inline (beg end str &optional no-insert)
  "Insert Inline Inline (general)."
  (if str
      (let ((insstr (concat beg str end)))
        (if no-insert insstr (insert insstr)))
    (progn
      (if rd-use-prompt-when-insertion-p
          (rd-insert-inline beg end (read-string (concat beg " elm " end ": ")) no-insert)
        (insert beg end)
...
(defun rd-insert-ref (&optional str)
  ...
  (interactive "*") (insert (rd-refquote (rd-insert-inline "((<" ">))" str t))))

(defun rd-insert-ref-noquote (&optional str)
  "Insert Inline Reference."
  (rd-insert-inline "((<" ">))" str))
...
(defun rd-yank-as-url (&optional arg)
  ...
  (let ((yanked-str
	 (concat "((<URL:" (buffer-substring (point) (mark)) ">))")))
...
(defun rd-insert-url (url label)
  ...
  (if (string= label "")
    ...
    (rd-insert-ref-noquote (concat (rd-refquote label) "|URL:" url))))

;;; -> GC  
;(defun rd-search-labels ()
;  "Search Label of RD for abbrev."
;  (interactive)
;  (save-excursion
;    (save-restriction
;      (save-match-data
...
;	      (if (or (string= label "begin") (string= label "end"))
;		  nil
;		(nadd-assoc label-list
;			    (cons label (point))))))
;	  label-list)))))
;
;(defun nadd-assoc (dict item)
;  "add new item into assoc Destructively."
;  (if (null (car dict))
;     (setcar dict item)
;    (if (assoc (car item) dict)
;	dict
;      (nconc dict (list item)))))
